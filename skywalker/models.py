...
def transit_model_func(model_params, times, init_t0=0.0, ldtype='quadratic', transitType='primary'):
    ...
    if 'deltaTc' in model_params.keys():
        if 'deltaEc' in model_params.keys():
            bm_params.t_secondary = model_params[
                'deltaTc'] + init_t0 + 0.5 * model_params['period'] + model_params['deltaEc']
        else:
            bm_params.t_secondary = model_params[
                'deltaTc'] + init_t0 + 0.5 * model_params['period']
    else:
...
def line_model_func_multi(model_params, ntransits, transit_indices, times):
    ...
    for k in range(ntransits):
        intercepts.append(model_params['intcept{}'.format(
            k)] if 'intcept{}'.format(k) in model_params.keys() else 1.0)
        slope = model_params['slope{}'.format(k)] if 'slope{}'.format(
            k) in model_params.keys() else 0.0
        crvtur = model_params['crvtur{}'.format(k)] if 'crvtur{}'.format(
            k) in model_params.keys() else 0.0
...
def line_model_func(model_params, times):
    intercept = model_params[
        'intercept'] if 'intercept' in model_params.keys() else 1.0
    slope = model_params['slope'] if 'slope' in model_params.keys() else 0.0
    curvature = model_params[
        'curvature'] if 'curvature' in model_params.keys() else 0.0

    line_model = intercept
    line_model = line_model + slope * \
        (times - times.mean()) if slope is not 0.0 else line_model
    line_model = line_model + curvature * \
        (times - times.mean())**2 if curvature is not 0.0 else line_model
...
def phase_curve_func(model_params, times, init_t0):
    ...
    if 'period' not in model_params.keys():
        raise Exception('`period` not included in `model_params`')
    ...
    if 'cosPhase' in model_params.keys() and 'cosAmp' in model_params.keys():
        ...
        phase_curve = model_params['cosAmp'] * np.cos(ang_freq * (times - t_secondary)) + model_params[
            'sinAmp'] * np.sin(ang_freq * (times - t_secondary))
        ...
        if 'sinAmp2' in model_params.keys() and 'cosAmp2' in model_params.keys():
            phase_curve = model_params['cosAmp1'] * np.cos(ang_freq * (times - t_secondary)) + model_params['sinAmp1'] * np.sin(ang_freq * (times - t_secondary)) + \
                model_params['cosAmp2'] * np.cos(2 * ang_freq * (times - t_secondary)) + model_params[
                'sinAmp2'] * np.sin(2 * ang_freq * (times - t_secondary))
        else:
            phase_curve = model_params['cosAmp1'] * np.cos(ang_freq * (times - t_secondary)) + model_params[
                'sinAmp1'] * np.sin(ang_freq * (times - t_secondary))
    else:
...
def compute_sensitivity_map(model_params, method, xcenters, ycenters, residuals, knots, nearIndices, xBinSize, yBinSize, ind_kdtree, gw_kdtree, pld_intensities, model):
    if 'bliss' in method.lower():
        ...
        sensitivity_map = bliss.BLISS(xcenters, ycenters, residuals, knots,
                                      nearIndices, xBinSize=xBinSize, yBinSize=xBinSize, normFactor=normFactor)
    elif 'krdata' in method.lower():
        sensitivity_map = np.sum(residuals[ind_kdtree] * gw_kdtree, axis=1)
    elif 'pld' in method.lower():
        PLDcoeffs = [val.value for val in model_params.values()
    ...
    nSig = 10
    vbad_sm = np.where(abs(sensitivity_map - np.median(sensitivity_map))
                       > nSig * scale.mad(sensitivity_map))[0]
...
def add_line_params(model_params, phase, times, transitType='primary'):
    ...
    ph_where_transits = np.where(
        abs(ph_diff_times) > nSig * std_ph_diff_times)[0]
...
def add_pld_params(model_params, fluxes, pld_intensities,
    ...
    for k in range(order):
        assert(np.allclose(pld_intensities[
               :n_pld]**(k + 1), pld_intensities[k * n_pld:(k + 1) * n_pld]))
    ...
    pld_coeffs = np.linalg.lstsq(pld_intensities, fluxes)[0] if not start_unity else np.ones(
    ...
    # if add_unity: model_params.add_many(('pld{}'.format(n_pld_out),
    # pld_coeffs[n_pld_out], True)) # FINDME: Maybe make min,max = 0,2 or =
    # 0.9,1.1
    if add_unity:
