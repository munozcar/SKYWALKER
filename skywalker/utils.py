...
def extractData(file, flux_key='phots', time_key='times', flux_err_key='noise',
                eff_width_key='npix', pld_coeff_key='pld',
                ycenter_key='ycenters', xcenter_key='xcenters',
...
def removeOutliers(xcenters, ycenters, fluxes=None, x_sigma_cutoff=3,
    ...
    """
    x_ell = ((xcenters - xcenters.mean()) /
             x_sigma_cutoff)**2.  # x-ellipse term
    y_ell = ((ycenters - ycenters.mean()) /
             y_sigma_cutoff)**2.  # y-ellipse term
...
def setup_inputs_from_file(dataDir, x_bin_size=0.1, y_bin_size=0.1,
                           xSigmaRange=4, ySigmaRange=4, fSigmaRange=4,
                           flux_key='phots', time_key='times',
                           flux_err_key='noise', eff_width_key='npix',
                           pld_coeff_key='pld', ycenter_key='ycenters',
                           xcenter_key='xcenters', ywidth_key='ywidths',
                           xwidth_key='xwidths', method=None):
    """
    Description:
        This function takes in the filename of the data (stored with 
        sklearn-joblib), checks the data for outliers, establishes the 
        interpolation grid, computes the nearest neighbours between all data 
        points and that grid, and outputs the necessary values for using 
        BLISS. The 'flux' is assumed to be pure stellar signal -- i.e. no 
        planet. BLISS is expected to be used inside a fitting routine where 
        the transit has been 'divided out'. This example here assumes that 
        there is no transit or eclipse in the light curve data (i.e. `flux` == 
        'stellar flux'). To use this with a light curve that contains a 
        transit or eclipse, send the "residuals" to BLISS: - i.e. `flux = 
        system_flux / transit_model`
    ...
    extracted_data = extractData(file=dataDir,
                                 flux_key=flux_key,
                                 time_key=time_key,
    ...
    keep_inds = removeOutliers(xcenters, ycenters, fluxes=None,
    ...
    if 'bliss' in method.lower():
        ...
        knots = bliss.createGrid(xcenters[keep_inds], ycenters[keep_inds],
                                 x_bin_size, y_bin_size)

        print('BLISS will use a total of {} knots'.format(len(knots)))
        knotTree = spatial.cKDTree(knots)
        nearIndices = bliss.nearestIndices(xcenters[keep_inds],
                                           ycenters[keep_inds], knotTree)
...
def sub_ax_split(x, y, axs, markersize=5, alpha=0.5, lw=0, mew=0,
                 points_per_bins=10, histtype='stepfilled',
                 orientation='horizontal', color='gray', label=None,
                 returnHist=False):
    ax_scat, ax_hist = axs
    ax_scat.plot(x, y, '.', color=color, markersize=markersize,
                 alpha=alpha, mew=0, label=label)
    yhist, xhist, patches = ax_hist.hist(y, bins=y.size // points_per_bins,
                                         histtype=histtype, orientation=orientation, color=color, density=False)
...
def plot_fit_residuals_physics(times, fluxes, flux_errs, model_set,
