...
def line_model_func_multi(model_params, ntransits, transit_indices, times, token=False):
    intercepts = []
    coeffs_line_list = []
...
def phase_curve_func(model_params, times, init_t0):
    ...
    if 'cosPhase' in model_params.keys() and 'cosAmp' in model_params.keys():
        phase_curve = model_params['cosAmp']*np.cos(ang_freq * (times - t_secondary) + model_params['cosPhase'])
    ...
    return phase_curve - phase_curve.min() + 1.0
